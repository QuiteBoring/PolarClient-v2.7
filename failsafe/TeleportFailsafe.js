    ...
    constructor() {
        ...
        this.triggers = [
            ...
            register("packetReceived", (packet) => {
                ...
                if (this.toggle && playerPos.getDistance(packetPos) >= 0.1) Client.scheduleTask(6, () => {
                    if (this.isTeleporting() || !this.toggle) return
                    this.reset()

                    if (playerPos.getDistance(new Vector(Player.getX(), Player.getY(), Player.getZ())) <= 0.1)
                        return ChatUtils.sendCustomMessage("AutoVegetable", "&cTeleport detected but you were teleported back! No action taken.")
